...
- **`drawio-mcp-diagramming`**: Creating or updating Azure architecture diagrams. 
  Always follow `.github/skills/drawio-mcp-diagramming/SKILL.md` when generating 
  draw.io XML via the MCP server.

## Diagramming Standards

When creating Azure architecture diagrams using the draw.io MCP server:

1. **Always read** `.github/skills/drawio-mcp-diagramming/SKILL.md` before generating XML
2. **Azure icons** must use `img/lib/azure2/` paths, never the legacy `azure/` set
   - Full style: `image;aspect=fixed;whiteSpace=wrap;html=1;image=img/lib/azure2/<folder>/<Icon>.svg;`
   - Common paths:
     ...
     - Azure Monitor              → `management/Monitor.svg`
     - Log Analytics Workspace    → `analytics/Log_Analytics_Workspaces.svg`
     - AKS                        → `compute/Kubernetes_Services.svg`
     - DNS Zone                   → `networking/DNS_Zones.svg`
     - Web Application Firewall   → `networking/Web_Application_Firewall_Policies.svg`
     - Azure AD / Entra ID        → `identity/Azure_Active_Directory.svg`
     - Blob Storage               → `storage/Storage_Accounts.svg`
     - Azure SQL                  → `databases/SQL_Databases.svg`
     - Virtual Machines           → `compute/Virtual_Machines.svg`
     - App Service                → `compute/App_Services.svg`
     - Azure Firewall             → `networking/Firewalls.svg`
     - Private Endpoint           → `networking/Private_Endpoint.svg`
3. **Azure Monitor, Log Analytics, Microsoft Sentinel, Defender for Cloud** must always
   be placed OUTSIDE VNet/subnet containers — these are subscription-scoped or
   workspace-scoped services, not VNet-bound
4. **XML must be indented** — one `mxCell` per line, never minified
5. **Validate** with `xmllint --noout <file>` before saving

Here's the equivalent instruction set for AWS architecture diagrams:
instructions## Diagramming Standards

When creating AWS architecture diagrams using the draw.io MCP server:

1. **Always read** `.github/skills/drawio-mcp-diagramming/SKILL.md` before generating XML
2. **AWS icons** must use `img/lib/aws4/` paths, never the legacy `aws/` or `aws3/` sets
   - Full style: `image;aspect=fixed;whiteSpace=wrap;html=1;image=img/lib/aws4/<folder>/<Icon>.svg;`
   - Common paths:
     - VPC                  → `networking_content_delivery/VPC.svg`
     - Application Load Balancer → `networking_content_delivery/Application_Load_Balancer.svg`
     - Secrets Manager      → `security_identity_compliance/Secrets_Manager.svg`
     - CloudWatch           → `management_console/CloudWatch.svg`
     - CloudWatch Logs      → `management_console/CloudWatch_Logs.svg`
     - EKS                  → `containers/Elastic_Kubernetes_Service.svg`
     - Route 53             → `networking_content_delivery/Route_53.svg`
     - WAF                  → `security_identity_compliance/WAF.svg`
     - IAM                  → `security_identity_compliance/Identity_and_Access_Management.svg`
     - S3                   → `storage/S3.svg`
     - RDS                  → `database/RDS.svg`
     - EC2                  → `compute/EC2.svg`
3. **CloudWatch, CloudTrail, AWS Config, Security Hub** must always be placed OUTSIDE
   VPC/subnet containers — these are regional/global services, not VPC-bound
4. **XML must be indented** — one `mxCell` per line, never minified
5. **Validate** with `xmllint --noout <file>` before saving
